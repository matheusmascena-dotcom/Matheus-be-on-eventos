(() => {
  ...
  let revealed = false;
  const reveal = () => { if(revealed)return; revealed=true; document.documentElement.style.visibility=''; };
  ...
  const sb=window.supabase?.createClient?.(SUPABASE_URL,SUPABASE_KEY);if(!sb){reveal();return;}
  async function run(){try{
    const {data:e,error}=await sb.from('events').select('*').eq('slug',slug).maybeSingle();if(error||!e){reveal();return;}
    ...
    const purchaseUrl=PURCHASE_URLS[e.slug]||e.purchase_url;if(purchaseUrl)schema.offers={'@type':'Offer',url:purchaseUrl,availability:'https://schema.org/InStock'};Object.keys(schema).forEach(key=>schema[key]===undefined&&delete schema[key]);setJsonLd(schema);
    const cover=qs('#cover');if(cover){cover.removeAttribute('src');cover.src=e.image_url||'';cover.alt=e.name;}if(qs('#name'))qs('#name').textContent=e.name;if(qs('#date'))qs('#date').textContent=new Date(e.event_date+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'});if(qs('#local'))qs('#local').textContent=e.location||'';if(qs('#artists'))qs('#artists').textContent=e.artists||'';if(qs('#buy'))qs('#buy').href=purchaseUrl||'#';const map=qs('#maps');if(map)map.href=e.source_url||'#';
    const {data:photos}=await sb.from('event_gallery_photos').select('*').eq('event_id',e.id).order('position');const list=photos||[];const gallery=qs('#gal'),credits=qs('#credits'),section=qs('#gallery');if(section){if(!list.length)section.style.display='none';else{gallery.innerHTML=list.slice(0,8).map((p,i)=>`<a class="galitem" data-i="${i}"><img src="${esc(p.src_url)}" alt="${esc(p.photo_name||e.name)}" loading="lazy"></a>`).join('');const uniq=[];list.forEach(p=>{if(p.credit_label&&!uniq.some(c=>c.label===p.credit_label))uniq.push({label:p.credit_label,url:p.credit_url});});credits.innerHTML=uniq.map(c=>c.url?`<a href="${esc(c.url)}" target="_blank" rel="noopener">📷 ${esc(c.label)}</a>`:`📷 ${esc(c.label)}`).join(' · ');window.__beonPhotos=list;document.querySelectorAll('.galitem').forEach(a=>a.onclick=()=>window.__beonOpen?.(+a.dataset.i));}}
    const target=new Date(e.event_date+'T00:00:00-03:00').getTime();const tick=()=>{let x=Math.max(0,target-Date.now()),d=Math.floor(x/86400000);x%=86400000;const h=Math.floor(x/3600000);x%=3600000;const m=Math.floor(x/60000),s=Math.floor(x/1000)%60;[['d',d],['h',h],['m',m],['s',s]].forEach(([id,value])=>{const el=qs('#'+id);if(el)el.textContent=String(value).padStart(2,'0');});};tick();setInterval(tick,1000);
    try{await sb.from('site_metrics').insert({metric_type:'event_view',event_id:e.id,path:location.pathname+location.search,referrer:document.referrer||null,user_agent:navigator.userAgent});}catch(_){ }
  }finally{reveal();}}
  const lb=qs('#lb'),img=qs('#lbimg'),counter=qs('#counter');let idx=0;window.__beonOpen=i=>{const photos=window.__beonPhotos||[];if(!photos.length)return;idx=i;lb?.classList.add('on');show();};function show(){const photos=window.__beonPhotos||[];if(!photos[idx])return;img.src=photos[idx].src_url;counter.textContent=`${idx+1}/${photos.length}`;}function move(n){const photos=window.__beonPhotos||[];if(!photos.length)return;idx=(idx+n+photos.length)%photos.length;show();}qs('.close')?.addEventListener('click',()=>lb?.classList.remove('on'));qs('.prev')?.addEventListener('click',()=>move(-1));qs('.next')?.addEventListener('click',()=>move(1));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
  try{if(!window.__beonAnalyticsLoader){window.__beonAnalyticsLoader=true;const s=document.createElement('script');s.src='assets/beon-analytics-v1.js?v=14202d3e';s.defer=true;document.head.appendChild(s);}}catch(_){ }
  try{if(!window.__beonSocialVideosLoader){window.__beonSocialVideosLoader=true;const s=document.createElement('script');s.src='assets/beon-social-videos-v1.js?v=1';s.defer=true;document.head.appendChild(s);}}catch(_){ }
})();