(() => {
  ...
  const rawSlug=new URLSearchParams(location.search).get('event'); if(!rawSlug)return;
  const aliases={'unreal-the-grid':'unreal-x-the-grid'};
  ...
  async function run(){
    ...
    const buy=qs('#buy'); if(buy){buy.href=e.purchase_url||'#'; buy.dataset.eventSlug=e.slug;}
